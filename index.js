...
function bin() {
  ...
  histogram.thresholds = function(_) {
    return arguments.length ? (threshold = typeof _ === "function" ? _ : constant$3(Array.isArray(_) ? slice.call(_) : _), histogram) : threshold;
  };
...
class VerticalRidgePlot extends RidgePlot {
  ...
  render(width = 400, height = 400) {
    ...
    var x = band().range([0, width]).domain(this._dkeys).padding(0.2);
    ...
    if ("yminmax" in this.state) {
      ...
      yminmax = extent(this._dentries.map((x) => Math.max(...x[1].values)));
    }

    var y = linear().domain([0, yminmax[1]]).range([height, 0]).nice();
...
class HorizontalRidgePlot extends RidgePlot {
  ...
  render(width, height) {
    ...
    var y = band().range([0, height]).domain(this._dkeys).padding(0.2);
    ...
    let yAxis = svg.append("g").call(axisLeft(y));
    ...
    if ("xminmax" in this.state) {
      ...
      xminmax = extent(this._dentries.map((x) => Math.max(...x[1].values)));
    }

    var x = linear().domain([0, xminmax[1]]).range([0, width]).nice();
...
export { HorizontalRidgePlot, VerticalRidgePlot };

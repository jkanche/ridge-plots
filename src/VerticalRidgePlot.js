...
export class VerticalRidgePlot {
  /**
  ...
  _check_for_data() {
    if (
      ...
      throw `data is either not set or is not a valid type!`;
    }
  }
  ...
  render(width = 400, height = 400) {
    ...
    const yminmax = d3.extent(
      this._dentries.map((x) => Math.max(...x[1].values))
    );
    ...
    function getColor(i) {
      if (typeof colorScale === "string" || colorScale instanceof String) {
        ...
        return colorScale(data[d.key][this.state.metric]);
      }
    }
    ...
    svg
      ...
      .on("mouseover", function (event, d) {
        const idx = dkeys.indexOf(d.key);
        const mets = dentries[idx][1];
